...
const TrainingScheduleDialogWizard = ({ open, onOpenChange, onSaved, editingData }: TrainingScheduleDialogProps) => {
  ...
  useEffect(() => {
    if (open) {
      setHasSubmitted(false); // Reset submission guard when dialog opens
      if (editingData?.id) {
        ...
        // Map existing training days to week plans
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let dayCounter = 1;
      console.debug(`[Dialog] Building training days from week plans...`);
      ...
      for (let week = 1; week <= 4; week++) {
        for (let dayOfWeek = 1; dayOfWeek <= 7; dayOfWeek++) {
          ...
          if (groupId) {
            ...
            console.debug(`[Dialog] Day ${dayCounter}: No workout assigned (week=${week}, dayOfWeek=${dayOfWeek})`);
            dayCounter++;
          }
        }
      }
      
      console.debug(`[Dialog] Total training days in payload: ${trainingDaysData.length}`);

      // Check for duplicates in trainingDaysData
      const groupKeySet = new Set<string>();
      const duplicateGroups = new Map<string, number[]>();
      trainingDaysData.forEach((day: Record<string, unknown>) => {
        const exerciseGroup = day.exerciseGroup as Record<string, unknown>;
        const exerciseMethods = exerciseGroup?.exerciseMethods as unknown[] | undefined;
        const methodCount = Array.isArray(exerciseMethods) ? exerciseMethods.length : 0;
        const groupName = typeof exerciseGroup?.name === 'string' ? exerciseGroup.name : '';
        const categoryId = exerciseGroup?.categoryId;
        const groupKey = `${groupName}|${categoryId || 'null'}|${methodCount}`;
        
        if (!duplicateGroups.has(groupKey)) {
          duplicateGroups.set(groupKey, []);
        }
        const dayNum = typeof day.day === 'number' ? day.day : 0;
        duplicateGroups.get(groupKey)!.push(dayNum);
        groupKeySet.add(groupKey);
      });

      console.debug(`[Dialog] Unique groups in payload: ${groupKeySet.size}`);
      duplicateGroups.forEach((days, groupKey) => {
        console.debug(`[Dialog] Group "${groupKey}": ${days.length} days (${days.join(', ')})`);
      });
      ...
      console.debug(`[Dialog] Final payload:`, JSON.stringify({
        name: payload.name,
        publicName: payload.publicName,
        trainingDaysCount: payload.trainingDays.length,
        trainingDays: payload.trainingDays.map(d => ({
          day: d.day,
          shortName: d.shortName,
          groupName: d.exerciseGroup.name,
          categoryId: d.exerciseGroup.categoryId,
          methodCount: d.exerciseGroup.exerciseMethods.length,
        }))
      }, null, 2));
