...
import { fadeUpIn, listContainer, listItem, hoverLift, hoverScale, tapScale, gridContainer, gridItem } from "@/lib/motion-variants";

// CLEANUP: Added interface for counts data
interface Counts {
  exercises: number;
  methods: number;
...
// CLEANUP: Added interface for quick access items
...
// CLEANUP: Added interface for activity items
...
const Home = () => {
  ...
  // CLEANUP: Removed unused 'loading' state - useLoading hook is sufficient
  const [counts, setCounts] = useState<Counts>({
    exercises: 0,
    methods: 0,
    workoutSheets: 23, // Placeholder
    trainings: 8 // Placeholder
  });

  // State for recent activity
  const [recentActivity, setRecentActivity] = useState<ActivityItem[]>([]);

  // CLEANUP: Extract loadCounts outside useEffect to make it stable and reusable
  const loadCounts = async (): Promise<void> => {
    startLoading();
    try {
      // CLEANUP: Fetch data in parallel
  ...
  useEffect(() => {
    ...
    // CLEANUP: Refresh data every 5 seconds to keep counts up-to-date
  ...
  // CLEANUP: Typed quickAccessItems array
  ...
  return (
    <Layout>
      ...
      >
        ...
        {/* CLEANUP: Simplified grid animation - removed redundant motion.div wrapper */}
        <motion.div 
