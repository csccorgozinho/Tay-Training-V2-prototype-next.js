...
const WorkoutSheets = () => {
  ...
  const [editingWorkoutSheet, setEditingWorkoutSheet] = useState<ExerciseGroup | null>(null);
  ...
  const handleRefresh = () => {
    // Trigger filter hook to refresh the data
    refreshSheets();
  };
  ...
  const handleOpenEditDialog = async (sheet: WorkoutSheetTransformed) => {
    try {
      ...
      setEditingWorkoutSheet(fullGroup);
    } catch (err) {
      console.error('Failed to fetch full sheet data:', err);
      // Fallback: set as null since we can't fetch the full data
      setEditingWorkoutSheet(null);
    }
    setWorkoutSheetDialogOpen(true);
  };

  const handleAddNewWorkoutSheet = () => {
    setEditingWorkoutSheet(null);
    setWorkoutSheetDialogOpen(true);
  };
  ...
  const handleDeleteWorkoutSheet = async (id: number) => {
    if (!confirm('Tem certeza que deseja deletar este grupo de exercícios?')) return;
    try {
      await apiDelete(`/api/exercise-groups/${id}`);
      handleRefresh();
      toast({
        title: "Grupo de exercícios excluído",
        description: "O grupo foi excluído com sucesso.",
      });
    } catch (err) {
      console.error(err);
      toast({ title: 'Erro', description: 'Não foi possível deletar o grupo de exercícios.' });
    }
  };
    ...
    <Layout>
      <div className="w-full animate-fade-in">
        <div className="py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8">
            ...
            <Button 
              ...
              onClick={handleAddNewWorkoutSheet}
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem 
                        ...
                        onClick={() => handleDeleteWorkoutSheet(sheet.id)}
                      >
          ...
          {filteredWorkoutSheets.length === 0 && !loading && !filterLoading && (
        ...
        <WorkoutSheetDialog 
          ...
          onOpenChange={setWorkoutSheetDialogOpen}
