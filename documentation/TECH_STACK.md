...
|------------|---------|------|-----------|
| Next.js | 14.0.0 | Full-stack framework | Pages, API routes, SSR |
...
| Zustand | 5.0.8 | State mgmt | Global loading state |
| React Query | 5.56.2 | Server state | API caching, dedup |
| Tailwind CSS | 3.4.11 | Styling | Utility-first CSS |
| Radix UI | ~1-2.x | UI primitives | Accessible components |
...
| NextAuth | 4.24.13 | Authentication | Login, sessions |
...
| Lucide React | 0.462.0 | Icons | UI icons |
...
- **Node Version:** The project uses `bun` as the package manager (indicated by `bun.lockb`), though npm is also supported
- **Build Output:** `.next/` directory contains optimized build artifacts
- **API Routes:** Serverless functions automatically deployed on platforms like Vercel, AWS Lambda, etc.
- **Database:** PostgreSQL connection via `DATABASE_URL` environment variable
- **Environment:** `.env` file stores secrets (not committed to git, as per `.gitignore`)
